# Gradient Boosting Classification for MyAnimeList Hits

This project uses Gradient Boosting to classify whether an anime will be a "Hit" (score > 8.0) or a "Standard Release" based on features like members, popularity, episodes, ranked, and genres.
...
2.  **Execution**: Run the main script to train the models and generate visualizations:
    ...
    - Train Gradient Boosting with **multiple hyperparameter configurations**.
...
1.  **ROC & PR Curves** (`gb_comparison_curves.png`): Compare different configurations.
2.  **Confusion Matrix** (`gb_confusion_matrix.png`): Shows true/false positives and negatives.
3.  **Feature Importance** (`gb_feature_importance.png`): Based on built-in feature importance.
