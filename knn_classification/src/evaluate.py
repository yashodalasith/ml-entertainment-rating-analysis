...
def plot_comparison_curves(y_test, results, filename='knn_comparison_curves.png'):
    """
...
def plot_confusion_matrix(y_test, y_pred, filename='knn_confusion_matrix.png'):
    """
...
def plot_feature_importance(model, X_test, y_test, feature_names, filename='knn_feature_importance.png'):
    """
    Calculates and plots permutation importance for KNN.
    """
    ...
    plt.barh(np.array(feature_names)[sorted_idx], perm_importance.importances_mean[sorted_idx], color='darkorange')
    plt.title("Permutation Importance — KNN (Top 10 Features)")
