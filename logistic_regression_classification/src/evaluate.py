...
def plot_comparison_curves(y_test, results, filename='lr_comparison_curves.png'):
    """
...
def plot_confusion_matrix(y_test, y_pred, filename='lr_confusion_matrix.png'):
    """
...
def plot_feature_importance(model, X_test, y_test, feature_names, filename='lr_feature_importance.png'):
    """
    Plots the coefficient-based feature importance for Logistic Regression.
    """
    print("Computing feature importance from coefficients...")
    importances = np.abs(model.coef_[0])
    sorted_idx = importances.argsort()[-10:]  # Top 10 features

    plt.figure(figsize=(10, 6))
    plt.barh(np.array(feature_names)[sorted_idx], importances[sorted_idx], color='steelblue')
