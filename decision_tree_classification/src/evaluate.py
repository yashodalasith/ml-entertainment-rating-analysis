...
def plot_comparison_curves(y_test, results, filename='dt_comparison_curves.png'):
    """
...
def plot_confusion_matrix(y_test, y_pred, filename='dt_confusion_matrix.png'):
    """
...
def plot_feature_importance(model, X_test, y_test, feature_names, filename='dt_feature_importance.png'):
    """
    Plots the Gini-based feature importance for Decision Tree.
    """
    print("Computing feature importance from tree structure...")
    importances = model.feature_importances_
    sorted_idx = importances.argsort()[-10:]  # Top 10 features

    plt.figure(figsize=(10, 6))
    plt.barh(np.array(feature_names)[sorted_idx], importances[sorted_idx], color='forestgreen')
